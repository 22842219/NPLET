...
from experiment_logger import commet_logger_args, CometLogger, NullLogger
...
@click.group()
@click.option(
    "--output-dir", default=os.path.join("output", "exp_" + "".join('ontonotes_modified')), type=click.Path())
...
def cli(ctx, **kwargs):
    ...
    if args.local_rank == -1 and args.num_gpus > 1:
        ...
        for args.local_rank in range(0, args.num_gpus):
            ...
            cmd = [sys.executable, "-u", "-m", "cli", "--local-rank={}".format(args.local_rank)]
...
from main import cli as entity_typing_cli
