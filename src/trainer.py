...
class Trainer(object):
    ...
    def writer(self):
        train_log = SummaryWriter(os.path.join(self.args.output_dir, "log", "train"))
        validation_log = SummaryWriter(os.path.join(self.args.output_dir, "log", "validation"))
            ...
            while True:
                for step, batch in enumerate(self.dataloader):
                    ...
                    if (step + 1) % self.args.gradient_accumulation_steps == 0:
                        ...
                        tensorboard.add_train_scalar('train_loss_', loss.item(), epoch)

                        global_step += 1

                        if self.step_callback is not None:
                            self.step_callback(model, global_step, tensorboard)
